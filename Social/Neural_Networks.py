#rete neurale classifier  su Most Used Platform
...
#ho delle stringhe, e posso togliere StudenId inutile
...
print(df[target].value_counts().sort_index()) #è sblianciato, però essendo social farei fatica a fare un raggruppamento sensato che non vada ad influenzare le predizioni
...
#faccio encoder per portare in numero le stringhe, features ed etichette
...
num_cols.remove('Student_ID')#rimuovo che è una features sensibile, ed inutile ai fini dell'addestramento
...
print(f"Colonne numeriche:{num_cols}")  #noto che manca il target, ottimo
...
print("Classi di 'status' presenti in Y_test: ", Y_test.unique())   #Stesso numero di classi tra test e train,bene

# Codifichiamo con dei numeri interi i valori delle classi di del target (es. 'Facebook': 1; 'Instagram': 2; eccetera){etichette target}
encoder= LabelEncoder()
Y_train_encoded= encoder.fit_transform(Y_train)
Y_val_encoded= encoder.transform(Y_val)
Y_test_encoded= encoder.transform(Y_test)

# Facciamo il one-hot encoding (li mettiamo a vettori) dei valori numerici che abbiamo ottenuto prima codificando le classi di target (es. Facebook: [0, 0, 1]; Instagram: [0, 1, 0]; eccetera)
...
disp= ConfusionMatrixDisplay(confusion_matrix=cm, display_labels=sorted(Y.unique()))   #ddisplay_labels=np.arange(cm.shape[0]), ho messo quello perchè ho avuto sfiga che nel test mancano alcune classi
...
#risultati strani, ma dovuti al dataset piccolo
