...
print(df[target].value_counts().sort_index()) #è sblianciato, tolgo 'Straps' unico elemento poco presente
df.drop(df[df[target] == 'Straps'].index, inplace=True) #tolgo quella classe (o se no facevio un repalce aggiundenolo/raggruppandolo ad un altra clòasse gia presente ma qua non c'èentrava cnulla)
...
num_cols.remove('target')#la rimuovo perchè non devo includere target nelle X (mi serviva per la matrice di correlazione e devo portare il target in numero)
...
#Essendo un classificatore, anche le feature con poca correlazione lineare le lascio, poichè potrebbero avere altre correlazioni/pattern nascosti che il classificatore è in grado di ricnoscere
...
Entrambe alte e stabili-->	Underfitting	Più neuroni, Più layer, Training più lungo(non riesce nemmeno ad impare i dati dell allenmaneto)
